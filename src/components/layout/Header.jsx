...
import { Bell, BellOff, LogOut, Clock, Download, User, ShieldCheck, Loader2 } from 'lucide-react';
    ...
    return (
        <header className="z-50 flex justify-between items-center px-6 sm:px-12 2xl:px-20 py-6 sm:py-10 w-full">
            <div className="flex items-center gap-6 sm:gap-12">
                <div className="flex flex-col">
                    <div className="flex items-baseline gap-2 sm:gap-4">
                        <h1 className="font-black text-white text-xl sm:text-3xl 2xl:text-5xl italic uppercase leading-none tracking-tighter">ATARAXIA</h1>
                        <div className="hidden xs:block bg-accent/10 px-2 py-1 border border-accent/20 rounded-lg" style={{ borderColor: `${accentColor}33` }}>
            ...
            <div className="flex items-center gap-3 sm:gap-6">
                {isInstallable && (
                    <button onClick={handleInstallClick} className="flex justify-center items-center bg-accent/10 hover:bg-accent/20 shadow-glow px-0 sm:px-6 border border-accent/30 rounded-xl sm:rounded-2xl w-11 sm:w-auto h-11 sm:h-14 text-accent active:scale-95 transition-all" style={{ color: accentColor, borderColor: `${accentColor}4d` }}>
                ...
                <button
                    ...
                    className={`flex justify-center items-center border rounded-xl sm:rounded-2xl w-11 sm:w-14 h-11 sm:h-14 transition-all active:scale-95 ${isGranted ? 'bg-accent/10 border-accent/30 text-accent shadow-glow' : 'bg-white/5 border-white/10 text-white/20'}`}
                    style={isGranted ? { color: accentColor, borderColor: `${accentColor}4d`, backgroundColor: `${accentColor}1a` } : {}}
                >
                    ...
                    <div className="flex items-center gap-3 bg-white/5 shadow-lg py-2 pr-2 pl-4 sm:pl-8 border border-white/10 rounded-xl sm:rounded-2xl">
                        <div className="hidden md:flex flex-col items-end">
                            <div className="flex items-center gap-2">
                                {profile.showIcon && <User size={12} className="opacity-60" style={{ color: accentColor }} />}
